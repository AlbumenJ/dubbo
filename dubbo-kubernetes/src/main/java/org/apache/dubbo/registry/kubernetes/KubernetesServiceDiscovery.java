        ...
        if (enableRegister) {
            kubernetesClient
                    ...
                    .edit(pod ->
                            new PodBuilder(pod)
                                    .editOrNewMetadata()
                                    .addToAnnotations(KUBERNETES_PROPERTIES_KEY, JsonUtils.getJson(applicationModel).toJson(serviceInstance.getMetadata()))
                                    .endMetadata()
                                    .build());
            ...
            for (EndpointAddress address : endpointSubset.getAddresses()) {
                ...
                instancePorts.forEach(port -> {
                    ...
                    if (StringUtils.isNotEmpty(properties)) {
                        serviceInstance.getMetadata().putAll(JsonUtils.getJson(applicationModel).toJavaObject(properties, Map.class));
